...
class Theta:
    ...
    def setMode(self, client_mode=True):
        ...
        self.baseurl = f'http://{self.baseip}'
    ...
    def showState(self, print_to_screen=True):
        ...
        if print_to_screen:
            print(f'All commands to: {self.baseurl}')
    ...
    def takePicture(self, print_to_screen=True):
        ...
        if 'error' in json_res:
            print(f'takePicture failed due to: {json_res["error"]["message"]}')
            return
    ...
    def downloadImage(self, image_url, directory, print_to_screen=True):
        ...
        if print_to_screen:
            ...
            if directory == '':
                ...
                print(f'- file saved here: {directory}')
        return saveto
    ...
    def waitUntilImageIsProcessed(self, print_to_screen=True):
        ...
        if self.num_images_taken > 0:
            if print_to_screen:
                print(f'{waittime} second timeout timer started for image processing:')
            while True:
                if print_to_screen:
                    print(
                        f'- time remaining: {round(timeout - time.time(), 2)}s, check status again '
                        f'in {check_status_timer}s')
    ...
    def connectToAP(self, ap_ssid=None, ap_sec='WPA/WPA2 PSK', ap_pw=None, print_to_screen=True):
        ...
        if print_to_screen:
            ...
            print(f'New static ip: {self.client_ip}')
        return res
    ...
    def getCaptureParameters(self, print_to_screen=True):
        ...
        if print_to_screen:
            print(
                f'Camera gain: {gain_exposure_tuple[0]} and exposure time [seconds]: {gain_exposure_tuple[1]}'
            )
    ...
    def getFileFormat(self, print_to_screen=True):
        ...
        if print_to_screen:
            print(f'The image format is: {format_res}')
        return format_res
    ...
    def yieldLivePreview(self, print_to_screen=True):
        ...
        if print_to_screen:
            print(f'Creating the live preview had HTTP response: {response}')
    ...
    def getLivePreviewAndWriteToDisk(self, file_name_prefix='livePreview', time_limit_seconds=10,
        ...
        for i, jpg in enumerate(self.yieldLivePreview(print_to_screen)):
            frameFileName = f'{file_name_prefix}.{i:04d}.jpg'
