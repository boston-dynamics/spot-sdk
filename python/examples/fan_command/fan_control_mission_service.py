...
from bosdyn.api import header_pb2, power_pb2
...
DIRECTORY_NAME = 'fan-off-callback'
...
class FanOffServicer(remote_service_pb2_grpc.RemoteMissionServiceServicer):
    """Turns fans on robot off

     Args:
        bosdyn_sdk_robot: SDK-Initialized Robot object
        fan_off_secs: Length of time in seconds that the fans will turn off for
        logger (logging.Logger): logging.Logger instance.
    """
    RESOURCE = 'body'

    def __init__(self, bosdyn_sdk_robot, fan_off_secs=10, logger=None):
        self.lock = threading.Lock()
        self.logger = logger or _LOGGER
        self.bosdyn_sdk_robot = bosdyn_sdk_robot
        self.fan_off_secs = fan_off_secs
        self.sessions_by_id = {}
    ...
    def _tick_implementation(self, request, response):
        """Make a fan power off request if none has been made; otherwise check request progress."""
        ...
        cmd_client = self.bosdyn_sdk_robot.ensure_client(PowerClient.default_service_name)
        ...
        #Perform Fan Off Command
        self._perform_and_monitor_fan_power_off(cmd_client, current_lease, request.session_id,
                                                response)

    def _perform_and_monitor_fan_power_off(self, cmd_client, lease, session_id, response):
        """Attempts to power off fans on the robot and check on fan command's progress
        ...
        if command_future is None:
            command_future = cmd_client.fan_power_command_async(percent_power=0,
                                                                duration=self.fan_off_secs,
        ...
        if command_future.done():
            ...
            try:
                command_id = command_future.result().command_id
            ...
            if feedback_future is None:
                feedback_future = cmd_client.fan_power_command_feedback_async(command_id)
            ...
            if feedback_future.done():
                ...
                feedback_status_codes = power_pb2.FanPowerCommandFeedbackResponse
                ...
                if (feedback_response.status == feedback_status_codes.STATUS_RUNNING):
                    response.status = remote_pb2.TickResponse.STATUS_RUNNING
                    # We need to issue another feedback request, so reset that future to None
                    # for the next tick.
                    self.sessions_by_id[session_id] = (command_future, None)
                elif (feedback_response.status == feedback_status_codes.STATUS_COMPLETE):
...
def run_service(bosdyn_sdk_robot, port, fan_off_secs=10, logger=None):
    ...
    service_servicer = FanOffServicer(bosdyn_sdk_robot, fan_off_secs, logger=logger)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--fan-off-secs', default=10,
        help='The number of seconds that the fan is turned off in the callback, default 10',
        type=int)
    ...
    service_runner = run_service(robot, options.port, fan_off_secs=options.fan_off_secs,
                                 logger=_LOGGER)
