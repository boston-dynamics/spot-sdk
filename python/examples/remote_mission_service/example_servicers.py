...
"""Examples of RemoteMissionServiceServicers."""
...
class PowerOffServicer(remote_service_pb2_grpc.RemoteMissionServiceServicer):
    ...
    def __init__(self, logger, sdk, robot_host, robot_username, robot_password):
    ...
    def _tick_implementation(self, request, response):
        ...
        cmd_client = self.robot.ensure_client(RobotCommandClient.default_service_name)
    ...
    def _establish_session_implementation(self, request, response):
        ...
        if self.robot is None:
            self.robot = self.sdk.create_robot(self.robot_host)
            try:
                self.robot.authenticate(self.robot_username, self.robot_password)
                ...
                response.header.error.message = 'Failed to authenticate user "{}"'.format(
                    self.robot_username)
                return
