    ...
    def GetImage(self, request, context):
        ...
        for img_req in request.image_requests:
            ...
            if frame is None or img_time is None:
                ...
                response.header.error.message = f'Failed to capture an image from {src_name} on the server.'
                return response
...
class CommsMapImageRenderer():
    ...
    def __init__(self, testmode, options):
        ...
        try:
            ...
            _LOGGER.error(f'Unable to create robot state client: {e}')
            sys.exit(-1)
    ...
    def put_results(self):
        ...
        try:
            ...
            _LOGGER.error('Unable to place comms survey map in frame: %s', e)
        ...
        for field_label in text_struct.keys():
            ...
            try:
                ...
                _LOGGER.error('Couldn\'t add line for %s because: %s', field_label, e)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--testmode', help='Run in \'test mode\' without using real comms data, return fake values',
        default='false')

    # base and service port arguments with CORE defaults
    parser.add_argument('--hostname', default='192.168.50.3',
                        help='Hostname or address of robot, e.g. '
                        '\'192.168.50.3\'')
