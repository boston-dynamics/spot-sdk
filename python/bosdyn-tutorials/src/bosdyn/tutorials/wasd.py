...
_ASCII_CHARS = ['@', '#', 'S', '%', '?', '*', '+', ';', ':', ',', '.']
_NUM_ASCII_CHARS = len(_ASCII_CHARS)
...
def _image_to_ascii(image, new_width):
    ...
    def _pixel_char(pixel_val):
        return _ASCII_CHARS[pixel_val * _NUM_ASCII_CHARS // 256]
...
class WasdInterface(object):
    ...
    def _request_power_on(self):
        self._power_client.power_command(PowerServiceProto.PowerCommandRequest.REQUEST_ON)
